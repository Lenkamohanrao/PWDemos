...
- Location: tests\accessibility.spec.ts:5:5
...
```ts
  ...
  4  | 
  5  | test("accesibility testing",async({page})=>{
  6  | await page.goto("https://demowebshop.tricentis.com/")
  7  | 
  8  | /* accesibilityScanResult=await new AxeBuilder({page}).analyze()
  9  | console.log("number of violation:==========>",accesibilityScanResult.violations.length)
  10 | 
  11 | expect(accesibilityScanResult.violations.length).toEqual(0);*/
  12 | 
  13 | const accesibilityScanResult=new AxeBuilder({page}).withTags(['wcag2a','wcag2aa','wcag21a','wcag21aa']).analyze();
  14 | console.log("number of tags violations:=====>",(await accesibilityScanResult).violations.length)
> 15 | expect((await accesibilityScanResult).violations.length).toEqual(0);
     |                                                          ^ Error: expect(received).toEqual(expected) // deep equality
  16 | 
  17 | 
  18 | 
